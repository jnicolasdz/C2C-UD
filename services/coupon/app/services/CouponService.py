...
class CouponService :
    ...
    def _coupon_code(coupon):
        if isinstance(coupon, dict):
            return coupon.get("code")
        return getattr(coupon, "code", None)
    ...
    def create_assigned_coupon(self, text: str, discount: float, days: int, user_email: str):
        ...
        self.user_db.add_assigned_coupon_to_user(user_email, self._coupon_code(new_coupon))
    ...
    def get_all_users_coupons(self):
        """
            ...
            Returns:
            - A dictionary where each key is a user's email and the value is another dictionary containing two lists.
        """
        users = self.user_db.get_all_users()
        return {
            user["email"]: {
                "assigned_coupons": user["assigned_coupons"],
                "unassigned_coupons": user["unassigned_coupons"],
            }
            for user in users
        }
    ...
    def delete_asigned_coupon_from_user(self, email: str, coupon_code: str):
        """
            Removes an assigned coupon from a user by moving it to unassigned history.
            This keeps compatibility with the original service behavior and exposes
            the DELETE endpoint for demo/admin actions.
        """
        user = self.user_db.get_user_by_email(email)
        if user is not None:
